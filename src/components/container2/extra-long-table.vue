<template>
  <div style="position:relative;height:100%">
    ...
    <div class="table-div" :style="{height:`${tableHeight}px`}">
      ...
      <div id="bottomDiv" class="table-bottom" @scroll.stop="handleScroll" v-show="showTableList.length>0">
        ...
        <table id="bottomTable" cellpadding="0" cellspacing="0" border="0" style="width:100%;table-layout:fixed;" :style="{height:`${loadedNum*tdHeight}px`}">
          ...
          >
            <td class="bottom-td" v-if="columns[0].type=='checkbox'" :style="columns[0].width?`width:${columns[0].width}px`:'width:auto'">
              <Checkbox v-model="items.checkbox" ></Checkbox>
            </td>
            <td v-for="(item,index) in columnsBottom" class="bottom-td" :style="item.width?`width:${item.width}px`:'width:auto'" v-html="item.logic==undefined?items[item.key]:item.logic(items)"></td>
    ...
    <div class="table-bottom-load" v-show="showLoad" :style="{right:`${isScroll}px`,left:0,top:'44px',height:`${tableHeight-40}px`}">
  ...
  computed: {
    ...
    columnsBottom() {
      ...
      for (let i = 0; i < this.columns.length; i++) {
        if (this.columns[i].type == undefined) {
          console.log(this.columns[i])
          ary.push(this.columns[i])
        }
      }
      return ary
    }
