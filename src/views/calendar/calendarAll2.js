        ...
        if (first === undefined) {
          ...
          if (first.during && first.during != 1) {
            ...
            while (rest > 0) {
              ...
              let nextweekarr = mounthdata[arrindex + count]
              for (let s = 0; s <_rest; s++) {
                ...
                //把数量数组改变
                themonthdaythingsnumber[arrindex + count][s]++;

                //改变一下一周的结果moutharr（总结过）
                montharr[arrindex + count][j] = [{ "colspan": _rest, "title": first.title, "className": first.type || '' }]
              }
            }
          }
        }
